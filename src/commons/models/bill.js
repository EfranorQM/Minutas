...
const eh = (fecha, dataView, asuntoView, desc) => {
  const perro = 'g';
  const result = {
    fecha: dateToApiFormat(fecha),
    idUsuarioSesion: perro.concat(dataView),
    atributos: 'entradaHuella',
    idAsunto: perro.concat(asuntoView),
    descripcion: desc,
  };
  return result;
};
