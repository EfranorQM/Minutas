          ...
          <q-card-section>
            ...
            >
              ...
              <div class="row">
                ...
                />
                <q-btn
                  v-if="!action.created && withFingerPrint.length < 1"
                  color="green"
...
export default {
  ...
  methods: {
    ...
    createHuella() {
      this.$q.dialog({
        title: 'Proceso para Registro de Huella',
        message: 'Coloca la huella dactilar, Recuerda que son 4 veces',
        cancel: true,
        ok: {
          label: 'Registar',
          color: 'positive',
        ...
        this.deletingFingerprint = true;
        await this.$store.dispatch('ui/crearHuella', this.action.id);
        this.withFingerPrint = await this.$store.dispatch('employees/withFingerPrint', this.action.id);
        this.deletingFingerprint = false;
      }).onOk(() => {})
        .onCancel(() => {})
